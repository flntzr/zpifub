...
public class StrategicMap {
	...
	int playerID = this.client.getMyPlayerNumber();
	int oppentID1 = (playerID + 1) % 3;
	int oppentID2 = (playerID + 2) % 3;
	this.powerupColor = 0xff << (oppentID1 * 8) | 0xff << (oppentID2 * 8);
	...
	for (int x = startX; x < endX; x++) {
		...
		boolean isWithinPowerdownArea = config.isWithinPowerdownArea(x, y);
		boolean isWithinPowerupArea = config.isWithinPowerupArea(x, y);
		if (isWithinPowerupArea || isWithinPowerdownArea) {
		    config.layer[0][x][y] = isWithinPowerdownArea ? 0 : powerupColor;
		} else {
	...
	if (layerN == 0) {
	    for (int x = 0; x < size; x++) {
		for (int y = 0; y < size; y++) {
		    boolean isWithinPowerdownArea = config.isWithinPowerdownArea(x + xOffset, y + yOffset);
		    boolean isWithinPowerupArea = config.isWithinPowerupArea(x + xOffset, y + yOffset);
		    if (isWithinPowerupArea || isWithinPowerdownArea) {
			config.layer[0][x + xOffset][y + yOffset] = isWithinPowerdownArea ? 0 : powerupColor;
		    } else {
	...
	// mapframe.setPixel((x/size) + offsetX, (y/size) + offsetY,
	// config.walklayer[l][x][y]);
	// }
	// }
	// //size = size << 1;
	...
	// mapframe.setPixel((x) + offsetX, (y) + offsetY,
	// config.walklayer[l][x/8][y/8]);
	// }
	// }
	// }
	// }
	...
	int size = 1;
	int offsetIncrease = 1024;
	int[][][] map = config.scoreHeatmap;
	for (int l = 0; l < 10; l++) {
	    if (l < 5) {
		for (int y = 0; y < map[l].length; y += size) {
		    for (int x = 0; x < map[l].length; x += size) {
			mapframe.setPixel((x / size) + offsetX, (y / size) + offsetY, map[l][x][y]);
		    }
		}
		// size = size << 1;
		if (l % 2 == 0)
		...
		if (l == 5)
		    ...
		    offsetY = (l - 4) * 140 + 50;
		for (int y = 0; y < map[l].length * 8; y++) {
		    for (int x = 0; x < map[l].length * 8; x++) {
			mapframe.setPixel((x) + offsetX, (y) + offsetY, map[l][x / 8][y / 8]);
		    }
		}
	    }
	}
